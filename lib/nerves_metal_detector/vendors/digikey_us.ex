defmodule NervesMetalDetector.Vendors.DigikeyUs do
...
defimpl NervesMetalDetector.Inventory.ProductAvailability.Fetcher,
  for: NervesMetalDetector.Vendors.DigikeyUs.ProductUpdate do
  alias NervesMetalDetector.Vendors.DigikeyUs

  def fetch_availability(%DigikeyUs.ProductUpdate{url: url, sku: sku}) do
    options = [
    ...
    with {:load_body, {:ok, %{body: body}}} when body not in [nil, ""] <-
      ...
      data = %{
        sku: sku,
        vendor: DigikeyUs.vendor_info().id,
        url: item_url,
        in_stock: in_stock,
        items_in_stock: nil,
